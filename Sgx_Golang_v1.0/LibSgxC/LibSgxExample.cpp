 ...
 */
extern "C" int cgo_initialize_enclave(void)
{
    ...
    fclose(fp);
    printf("enclave init successfully!\n");
    return 0;
}

/* Function: Destroy the enclave */
extern "C" void cgo_sgx_destroy_enclave(void)
{
    sgx_status_t ret = SGX_ERROR_UNEXPECTED;
...
extern "C" unsigned char* cgo_ecall_sgx_read_rand(int len, unsigned char *byteArrayPtr)
{    
    ...
    printf("C byte array char[%d] after SGX running:\n", len);
    for (int i = 0; i < len; i++) {
